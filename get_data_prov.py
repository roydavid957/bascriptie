...
#Descr:	collects and creates datafile used for bathesis without provinces in them
#Usage:	python get_data_prov.py
...
print("Descr:	collects and creates datafile used for bathesis without provinces in them")
print("Usage:	python get_data_prov.py")
...
for region in regions:
	...
	for user in users_region:
		if len(region_list) != 250:
			...
			#with open(region+".prov","w") as regionfile:
	...
	if len(random100_list) == 100:
		with open("random100.prov","w") as random100file:
	...
	if len(news_list) == 1000:
		...
		with open("news.prov","w") as newsfile: # data file used for bathesis experiments
